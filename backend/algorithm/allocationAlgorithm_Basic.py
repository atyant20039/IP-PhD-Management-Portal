...
for row in worksheet.iter_rows(min_row=2, values_only=True):
    ...
    room_no = room_no.strip().split(",")
    room_no = [str.strip() for str in room_no]

    # Create a new Course object and add the new course to the course list
    new_course = Course(accronynm, original_course_code, strength, date, time, day, room_no, building, room_capacity, TARatio)
...
for row in worksheet.iter_rows(min_row=2, values_only=True):
    ...
    if isinstance(row[course_code_col_idx - 1], str):
