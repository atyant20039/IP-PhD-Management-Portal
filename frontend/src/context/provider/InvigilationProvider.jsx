...
const API = import.meta.env.VITE_BACKEND_URL;
  ...
  const fetchData = async () => {
    try {
      ...
      const buildingRoomMapping = data.results.reduce((acc, curr) => {
        ...
        acc[curr.building].push({ roomNo: curr.roomNo, capacity: curr.capacity });
        return acc;
      }, {});
  ...
  const uploadData = async () => {
    ...
    try {
        const classroomXLSX = await convertDataToXLSX(classroom, "Classroom.xlsx");
        const TAXLSX = await convertDataToXLSX(TA, "TA.xlsx");
        const DatesheetXLSX = await convertDataToXLSX(Datesheet, "Datesheet.xlsx");
  ...
  return (
    <InvigilationContext.Provider
      value={{ buildingRoomMap,setCourseCode,setTARatio,TARatio,courseCode,setClassroom,setDatesheet,setuploadFile,setuploadFile1,setTA ,uploadData}}
    >
      {children}
    </InvigilationContext.Provider>
  );
