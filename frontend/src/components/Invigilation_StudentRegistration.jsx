...
function Invigilation_StudentRegistration({ onSubmit }) {
  const [studentRegistrationFile, setStudentRegistrationFile] = useState();
  ...
  const getFileTemplate = async () => {
    try {
      const response = await axios.get(`${API}/api/studentRegistrationList/`, {
        responseType: "blob",
      });
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API}/api/studentRegistrationList/`,
        formData,
        {
          headers: {
      ...
      <Card className="h-full w-full">
        ...
        >
          ...
          <div className="flex gap-4">
            <Tooltip content="Dowload Student Registration template file">
              <Button
                size="sm"
                variant="outlined"
                ripple={true}
...
export default Invigilation_StudentRegistration;
